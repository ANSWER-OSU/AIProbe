"""This script runs simulations for the ACAS XU environment using parallel processing."""
...
def initialize_and_run_simulations_parallel(base_folder, output_csv="simulation_results_parallel.csv", num_workers=4):
    ...
    with Manager() as manager:
        crash_counter = manager.dict({"count": 0})
...
if __name__ == "__main__":
    settings = ['accurate', 'inaccurate_reward', 'incomplete_state_rep', 'incomplete_state_and_reward']
    for setting in settings:
        SETTING = setting
        parent_directory = f"AIProbe/Result/ACAS_XU/Result/{N_BINS}_bin"
        num_workers =  1
        log_file = f"AIProbe/Result/ACAS_XU/Result/{N_BINS}_bin/{SETTING}_simulation_log.txt"
        ...
        with open(log_file, "w") as log:
            ...
            initialize_and_run_simulations_parallel(parent_directory, output_csv=f'peformance_with_{SETTING}.csv', num_workers=num_workers)
