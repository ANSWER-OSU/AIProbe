...
# ---- Configuration ----
BASE_DIR = "/scratch/projects/AIProbe-Main/Result/BIpedal/new_test/100_Bin"
...
MODEL_PATHS = {
    ...
    # Inaccurate PPO models
     "modified_obs": "/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/BipedalInaccuratePPOModels/ppo_bipedalwalker_modified_obs.zip",
     "modified_reward": "/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/BipedalInaccuratePPOModels/ppo_bipedalwalker_modified_reward.zip",
     "modified_obs_reward": "/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/BipedalInaccuratePPOModels/ppo_bipedalwalker_modified_obs_modified_reward.zip",
}
...
# Setup logging
logging.basicConfig(
    filename=os.path.join(BASE_DIR, "simulation_log_tqc.txt"),
    ...
    try:
        ...
        for attr in root.findall("Attribute"):
            ...
            if name_elem is not None and name_elem.attrib.get("value") == "Terrain":
                ...
                if not terrain_str:
                    print(f"⚠️ Warning: Empty terrain data in {xml_file}, using default [0]")
        ...
        print(f"⚠️ Warning: Terrain attribute not found in {xml_file}, using default [0]")
        return terrain_values
...
def run_simulation(xml_file,model_type):
    for i in range(1,2):
        ...
        # model = SAC.load("/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/rl-baselines3-zoo/rl-trained-agents/sac/BipedalWalkerHardcore-v3_1/BipedalWalkerHardcore-v3.zip", device="cpu")
        ...
        if model_type in model_cls_map:
            ...
            # For modified PPO models (with altered obs or reward)
        ...
        for episode in range(NUM_EPISODES):
            ...
            for i in range(N_TIMESTEPS):
                ...
                print('timestep: ', i, 'action: ', action)
                print('hull x:', base_env.hull.position.x, 'hull y:', base_env.hull.position.y)
...
#               with LOCK:
#                   with open("run_log", "a", newline="") as csvfile:
#                       csv_writer = csv.writer(csvfile)
#                       csv_writer.writerow([xml_file,action_seq])
#               
#               return True
...
if __name__ == "__main__":
    ...
    # === One model and one XML ===
    xml_file = "/scratch/projects/AIProbe-Main/Result/BIpedal/Approch_1_new/100_Bin/534/Env_50/Task_1/initialState.xml"
    model_type = "tqc"  # Choose from: "ppo", "sac", "td3", "a2c", "tqc", or your custom keys
    
    start_time = time.time()
    readable_start = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(f"🟢 Starting single simulation at {readable_start}")
    
    
    run_simulation(xml_file, model_type)
    
    end_time = time.time()
    readable_end = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    elapsed_minutes = (end_time - start_time) / 60
    
    logger.info(f"✅ Single simulation complete at {readable_end}")
    logger.info(f"🕒 Time taken: {elapsed_minutes:.2f} minutes")
...
#
## === Your model map and imports assumed to exist ===
#MODEL_TYPES = ["modified_obs"]
#CSV_PATH = "/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/crash_stats_per_seed.csv"  # CSV with column: Path
#
#def wrapper(task_dir):
#   """Run a task across all models, but skip others if any succeeds."""
#   initial_xml = os.path.join(task_dir, "initialState.xml")
#   #initial_xml = "/scratch/projects/AIProbe-Main/Result/BIpedal/Approch_1_new/100_Bin/534/Env_50/Task_1/initialState.xml"
#   
#   if not os.path.exists(initial_xml):
#       print(f"⚠️ Missing: {initial_xml}")
#       return False
#   
#   for model_type in MODEL_TYPES:
#       try:
#           result = run_simulation((initial_xml, model_type))
#           if result is True:
#               return True  # ✅ One model succeeded, skip the rest
#       except Exception as e:
#           print(f"❌ Error running {model_type} on {initial_xml}: {e}")
#           
#   return False  # No model succeeded
#
#def main():
#   # === FIXED: read raw paths directly from file ===
#   with open("/scratch/projects/AIProbe-Main/AIProbe/RL_BipedalWalker/crash_accros_all_model.csv", "r") as f:
#       task_dirs = [line.strip() for line in f if line.strip()]
#       
#   print(f"🔧 Loaded {len(task_dirs)} tasks")
#   
#   with multiprocessing.Pool(processes=42) as pool:
#       results = list(tqdm(pool.imap(wrapper, task_dirs), total=len(task_dirs)))
#       
#   success_count = sum(results)
#   print(f"\n✅ Total tasks solved by any model: {success_count}/{len(task_dirs)}")
#   
#if __name__ == "__main__":
#   main()
