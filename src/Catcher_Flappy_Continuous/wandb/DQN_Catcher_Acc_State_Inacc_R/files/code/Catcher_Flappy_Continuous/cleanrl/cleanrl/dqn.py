...
class Args:
    ...
    wandb_project_name: str = "FlappyBird_DQN"
...
class FlappyBirdQNetwork(nn.Module):
...
class CatcherQNetwork(nn.Module):
...
"""
    ...
    if args.env_id=='FlappyBird':
        QNetwork = FlappyBirdQNetwork
    elif args.env_id=='Catcher':
        QNetwork = CatcherQNetwork
    else:
        raise ValueError(f"Unsupported environment: {args.env_id}")
    ...
    for global_step in range(args.total_timesteps):
        if args.save_model:
            ...
            if global_step % CHECKPOINT_FREQUENCY == 0:
                torch.save(q_network.state_dict(), f"{wandb.run.dir}/catcher_agent_acc_state_inacc_r_2.pt")
                wandb.save(f"{wandb.run.dir}/catcher_agent_acc_state_inacc_r_2.pt", base_path=wandb.run.dir, policy="now")
