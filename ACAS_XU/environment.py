...
class ACASagent:
    def __init__(self, x, y, theta, acas_speed, setting='accurate'):
        ...
        if self.setting == 'accurate':
            ...
            elif self.setting == 'inaccurate_state':
            ...
            self.model = PPO.load(ppo_model_path, device="cuda" if torch.cuda.is_available() else "cpu")
...
class env:
    ...
    def reward_func(self):
        ...
        for j in range(100):
            ...
            states_seq.append(self.normalize_state([self.row, self.alpha, self.phi, self.Vown, self.Vint]))
