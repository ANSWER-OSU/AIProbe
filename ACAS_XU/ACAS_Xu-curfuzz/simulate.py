...
def fuzz_testing(fuzzer, args, time_of_fuzzer=0):  
    ...
    while len(fuzzer.corpus) > 0:
        ...
        if collide_flag:
            ...
            print(f"Total Environments (Including Mutations): {len(fuzzer.corpus)}")
        total_environments = len(fuzzer.corpus)
        print(f"Total Environments agent Ran on : {mutated_count}")
    ...
    for i in range(len(result)):
        for m in range(3):
            if m == 0:
                ...
                #ani.save(gif_name)
                ani.save(gif_name, writer="ffmpeg", fps=30, extra_args=['-vcodec', 'libx264'])
