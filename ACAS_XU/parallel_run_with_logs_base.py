...
#     is_terminate, is_invalide_state = run_simulation(
...
#         "Invalid Intial State": is_invalide_state,
...
#         #     results = pool.map(process_wrapper, task_data)
...
def process_simulation(xml_file_path, env_folder, task_folder, seed_number, bin_number, crash_counter, output_csv):
    """Process a single simulation and immediately append results to the CSV."""
    env_data = Environment(xml_file_path)
    ...
    # Update crash count if terminated
    if is_terminate:
        crash_counter["count"] += 1
        print(f"💥 Crash detected! Total crashes so far: {crash_counter['count']}")
    ...
    result = {
        ...
        "Task": task_folder,
        "Terminated": is_terminate,
        "Invalid Initial State": is_invalide_state,
        "Execution Time (s)": execution_time,
        "File Path": xml_file_path
    }
...
def process_wrapper(task_crash_counter_output):
    """Helper function to run process_simulation in parallel."""
    task, crash_counter, output_csv = task_crash_counter_output
    return process_simulation(*task, crash_counter, output_csv)


def initialize_and_run_simulations_parallel(base_folder, output_csv="simulation_results_parallel.csv", num_workers=4):
    """
    Recursively finds 'finalState.xml' in all subdirectories and runs simulations in parallel,
    saving results as they are completed.
    """
    with Manager() as manager:
        crash_counter = manager.dict({"count": 0})  # Shared crash counter

        tasks = find_xml_files(base_folder)
        print(f"🔍 Found {len(tasks)} simulation files in {base_folder}")

        # Pair each task with the crash_counter and output file
        task_data = [(task, crash_counter, os.path.join(base_folder, output_csv)) for task in tasks]

        # Run simulations in parallel
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            executor.map(process_wrapper, task_data)

        print(f"\n✅ Results saved to {os.path.join(base_folder, output_csv)}")
        print(f"\n💥 Final Total Crashes: {crash_counter['count']}")        

# Example Usage:
if __name__ == "__main__":

    parent_directory = "/scratch/projects/AIProbe/Test_Enviroment/ACAS_XU/534"
    #num_workers = os.cpu_count()
    num_workers = 100
    log_file = "simulation_log534.txt"
