...
def make_env(scenario_name, xml_path, inaccurate_model="original"):
    ...
    for i, agent in enumerate(world.agents):
        if str(i) in env_config.agents:
            agent.state.p_pos = np.array([env_config.agents[str(i)]["X"], env_config.agents[str(i)]["Y"]])
...
def test_policy_parallel(arglist, xml_config, model_type, trained_model, output_file):
    ...
    with U.single_threaded_session():
        ...
        if bug_found and termination_reason == "High Collision":
            crash_xml_path = os.path.join(os.path.dirname(final_xml_path), "crash.xml")
            shutil.copyfile(final_xml_path, crash_xml_path)
            update_full_environment_xml(crash_xml_path, env, step_id + 1)
        else:
            non_crash_xml_path = os.path.join(os.path.dirname(final_xml_path), "simulation_final.xml")
...
if __name__ == "__main__":
    ...
    base_folder = "/scratch/projects/AIProbe-Main/Result/MARL_Coop_Navi/100_bin_new/"
    ...
    with open(log_file_path, "a") as log_file:
        ...
        for seed_number in seeds:
            ...
            pool.starmap(test_policy_parallel, tasks)
            pool.close()
            pool.join()
