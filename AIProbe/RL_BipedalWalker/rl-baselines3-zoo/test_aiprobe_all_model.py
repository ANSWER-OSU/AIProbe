...
# ---- Output CSVs ----
OUTPUT_CSVS = {
    model_type: os.path.join(BASE_DIR, f"testing_simulation_results_{model_type}.csv")
...
def run_simulation(args):
    ...
    model = model_cls.load(MODEL_PATHS[model_type], env=None, custom_objects={
    ...
    for episode in range(NUM_EPISODES):
        ...
        with LOCK:
            with open(OUTPUT_CSVS[model_type], "a", newline="") as csvfile:
                ...
                writer.writerow([env_id, task_id, total_reward, i, termination_reason, bug])
...
if __name__ == "__main__":
    ...
    for model_type, output_csv in OUTPUT_CSVS.items():
        with open(output_csv, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["Env", "Task", "Total Reward", "Steps", "Termination Reason", "Bug Detected"])

    args_list = [(xml, model_type) for xml in xml_files for model_type in MODEL_PATHS]
