...
# import minigridEnv_old as MinigridEnv
import MinigridEnv
        ...
        for env_num in os.listdir(env_seed_path):
            ...
            for task in os.listdir(filepath):
                ...
                for inaccuracy_type, model in inaccuracy.items():
                    ...
                    print(f'Env: {env_number}, Seed: {seed}, Task: {task}')

                    output_path = f'Minigrid_testing/computePolicy/results_for_fuzzer_gen_configs/Env_{env_number}{model}.csv'
                    num_sim_trials = 1
                    ...
                    img = env.render()
                    if img is None:
                        raise ValueError("Render returned None. Check if your environment supports 'rgb_array' mode.")

                    image = Image.fromarray(img)
                    image.save("rendered_frame.png")

                    print('goal: ', env.goal_pos, env.goal_dir)
