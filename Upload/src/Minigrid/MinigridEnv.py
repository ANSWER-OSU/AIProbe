...
class CustomMiniGridEnv(MiniGridEnv):
    def __init__(self, environment_data, accurate_model=False, task='escLava', inaccuracy_type=None, goal_pos=None,
        ...
        example_state = (1, 1, 2, 0) if task == 'escLava' and inaccuracy_type in [0, 1] else (1, 1, 2)
        obs_dim = len(example_state)
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32)

        # Discrete actions: left, right, forward, pickup
        self.action_space = spaces.Discrete(len(self.valid_actions))

        self.goal_pos = goal_pos
        self.goal_dir = goal_dir

        self.max_steps = 10#4*(self.width)*(self.height)
    ...
    def step(self, action):
        # Call MiniGrid's logic (not used here directly, but you may need for rendering/logging)
        super().step(action)

        # Override reward and done if agent steps into Lava
        reward = 0
        terminated = False
        truncated = False
        ...
        # Custom reward for reaching goal (if any)
        if self.is_goal(self.agent_curr_state):
            reward = (1 - (0.9*(self.step_count/self.max_steps)))
            terminated = True

        # Return custom state as observation
        obs = np.array(self.agent_curr_state, dtype=np.float32)
        info = {}  # Must be a dict
        self.visited_states.add(self.agent_curr_state)

        # print(f"[Env {getattr(self, 'rank', 0)}] Step {self.step_count}, Pos: {self.agent_pos}, Action: {action}")
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.task=='escLava':
            if self.inaccuracy_type in set([2, 3]):
                ...
                if self.grid_list[self.agent_pos[0]][self.agent_pos[1]]!=None and self.grid_list[self.agent_pos[0]][self.agent_pos[1]].type=='lava':
                    self.agent_curr_state = (self.agent_pos[0], self.agent_pos[1], self.agent_dir, 1)
                else:
                    self.agent_curr_state = (self.agent_pos[0], self.agent_pos[1], self.agent_dir, 0)
        ...
        print(f"[Episode {self.episode_count}] Unique states visited: {(self.visited_states)}")
                    ...
                    if s!='wall':
                        if self.task=='escLava':
                            if self.inaccuracy_type in set([2, 3]):
                                ...
                                state_feature_rep.append((i, j, k, 1 if s=='lava' else 0))
                        elif self.task=='keyToGoal':
                            ...
                            for kp in key_picked:
                                if kp==False:
                                    if self.inaccuracy_type in set([2, 3]):
                                        ...
                                        state_feature_rep.append((i, j, k, kp, 'none', 1 if s=='lava' else 0))
                                elif kp==True:
                                    if self.inaccuracy_type in set([2, 3]):
                                        state_feature_rep.append((i, j, k, kp))
                                    else:
                                        for key_color in ['green', 'yellow', 'blue', 'grey', 'red']:
                                            state_feature_rep.append((i, j, k, kp, key_color, 1 if s=='lava' else 0))
    ...
    def get_reward(self, state, action):
        super().get_reward(state, action)
        
        x, y = state[0], state[1]
        if self.inaccuracy_type in (0, 2):
            if self.is_goal(state)==True:
                state_reward = (1 - (0.9*(self.step_count/self.max_steps)))
                return state_reward
            elif self.grid_list[x][y]!=None and self.grid_list[x][y].type=='lava':
                return -1
            else:
                return 0
                ...
                if s!='wall':
                    if self.task=='escLava':
                        if self.inaccuracy_type in set([2, 3]):
                            ...
                            new_lava_val = 1 if s=='lava' else 0
