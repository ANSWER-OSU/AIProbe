...
# === Parameters ===
# initial_xml = "/Volumes/External_ssd/Data/Flappy/Task_26/initialState.xml"
...
def run_dfs_survive_to_timesteps_k_sampling(S_i, target_timesteps, env_constructor, hash_fn, step_fn, is_crashing_fn,
    ...
    def dfs(S_curr_dict, instr_so_far, depth):
        ...
        attempts = 0
        max_attempts =  k  # cap random attempts per state
...
# from your_dfs_module import (
#     run_dfs_survive_to_timesteps_k_sampling,
#     make_env, hash_fn, step_fn,
#     is_crashing_fn, compute_steps_fn
# )

# === Parameters ===
base_dir = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/configurations/configurations"
csv_files = [
    "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/summary_gpt_configs_2.csv",
]

seeds = [10, 23, 66, 32, 73, 881, 71203, 93572, 28514, 60497]
action_space = [1, 1, 1, 1, 1, 1, 0]
max_depth = 200
b = 100
k = 10

# === Step 1: Get common buggy paths ===
...
#buggy_sets = [get_buggy_paths(file) for file in csv_files]
#common_buggy_paths = sorted(set.intersection(*buggy_sets))
#print(f"🧠 Found {len(common_buggy_paths)} common buggy paths.")

# === Step 2: Process one config ===
...
#     print(f"🚀 [{start_time.strftime('%Y-%m-%d %H:%M:%S')}] Running instruction generation in parallel with immediate logging...")



#     output_csv = "instruction_generation_summary.csv"
#     if os.path.exists(output_csv):
#         os.remove(output_csv)  # start fresh if needed

#     base_dir = "/scratch/projects/AIProbe-Main/Result/FlappyBird/Flappybird/Approch_1/100_Bin"
#     low_timestep_csv = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/low_timestep_bugs.csv"  # Your filtered CSV

#     # === Step: Load low timestep bugs and build paths ===
...
#     with Pool(processes=min(cpu_count(), 90)) as pool:
#         pool.starmap(wrapped_process, args)

#     end_time = datetime.datetime.now()
#     elapsed = end_time - start_time

#     print(f"✅ [{end_time.strftime('%Y-%m-%d %H:%M:%S')}] Instruction generation complete.")
#     print(f"⏱️ Total duration: {elapsed}")
...
if __name__ == "__main__":
    ...
    # === Setup logging ===
    log_file = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/instruction_generation_2.log"
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [%(levelname)s] %(message)s",
        handlers=[
            logging.FileHandler(log_file, mode='w'),
            # Uncomment below if you also want to log to console
    ...
    output_csv = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/instruction_generation_summary_2.csv"
    if os.path.exists(output_csv):
        os.remove(output_csv)

    base_dir = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/configurations/configurations"
    # base_dir = "/scratch/projects/AIProbe-Main/Result/FlappyBird/Flappybird/Approch_1/100_Bin"
    # low_timestep_csv = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/low_timestep_bugs.csv"
    low_timestep_csv = "/scratch/projects/AIProbe-Main/AIProbe/Catcher_Flappy_Continuous/state_coverage_analysis/summary_gpt_configs_2.csv"
    ...
    # Create full paths: base_dir/config_<NUM>
    common_buggy_paths = sorted([
        os.path.join(base_dir, config.strip())
        for config in df["ConfigIndex"]
    ])
    ...
    with Pool(processes=min(cpu_count(), 1)) as pool:
        pool.starmap(wrapped_process, args)
    ...
    log.info(f"⏱️ Total duration: {elapsed}")
